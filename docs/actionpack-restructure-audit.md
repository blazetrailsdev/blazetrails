...
Wave 7 (journey port) alone adds 1500–2000 LOC of routing engine,
and Wave 3 promotes `abstractcontroller/` to a logical top-level
package. Today actionpack tests are co-mingled with other no-DB
packages in a single `unit-tests` job; this section plans the
CI split that should ride alongside the restructure.
...
actionpack-tests:
  ...
  steps:
    ...
    - uses: ./.github/actions/setup # post-Phase 1.1 of ci-improvement-plan
...
from the `unit-tests` invocation). Doc-only first (this section);
...
### Interaction with `docs/ci-improvement-plan.md`

This plan layers cleanly on top of the existing CI roadmap:

- **Phase 1.1 (composite setup action).** The new
  `actionpack-tests` job should consume the same
  `./.github/actions/setup` composite once it ships. If this CI
  split lands first, it copies the install boilerplate and gets
  deduped in Phase 1.1.
- **Phase 1.2 (shared build artifact).** Actionpack tests need
  built `dist/` for its workspace deps (`arel`, `activesupport`,
  `activemodel`, `rack`). The shared build artifact strategy
  applies as-is.
- **Phase 2.1 (AR adapter matrix).** Independent of this work —
  actionpack stays out of the matrix entirely.
- **Phase 3 (path filters).** Once split, `actionpack-tests` can
  be gated on `packages/actionpack/**` changes (paired with a
  nightly full-matrix run). Out of scope for the Wave 1.5 PR.
...
300-LOC ceiling. Doc-only PR adding this section is the current
deliverable.
...
- **Setup overhead dominates if the job is fast.** A ~30s test
  ...
  Mitigation: wait for Phase 1.1 (composite setup) and Phase 1.2
  (shared build artifact) so the per-job fixed cost drops, then
  split. If splitting lands first, accept the wall-clock cost as
  a tradeoff for clearer signal.
...
1. Should `actionpack-tests` run on every PR, or be path-gated
   ...
   then gate in Phase 3.)
2. Where do cross-package integration tests actually live today —
   any under `packages/actionpack` that depend on AR fixtures? A
   quick `grep -rln "@blazetrails/activerecord" packages/actionpack/src`
   answers this; if non-empty, those tests need a home decision
   before the split.
...
4. Should `packages/actionview` also split out, or stay with the
   remaining `unit-tests` batch? (Out of scope here, but the
