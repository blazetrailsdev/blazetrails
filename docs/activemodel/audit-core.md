...
- §6 "ForcedMutationTracker#force_change: same clone-vs-no-clone pattern" — **rewritten.** Both implementations clone; the actual divergence is in the cloning _mechanism_ (Rails `value.clone` rescuing exceptions vs trails' `cloneValue` which preserves class instances unchanged).
