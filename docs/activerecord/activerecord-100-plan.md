...
Spec lives in [`tm-unification-plan.md`](archive/tm-unification-plan.md#phase-6). Replaces per-test `dropAllTables` with transactional BEGIN/ROLLBACK; promotes `beforeEach(defineSchema)` to module-level. Unblocked items can ship now; mechanical sweep gated on the infra switch.
