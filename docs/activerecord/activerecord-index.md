...
| [`query-logs-parity-plan.md`](query-logs-parity-plan.md)                 | plan: wire `QueryLogs` into the query pipeline (transformer registry, instrumentation reorder, `assertQueriesMatch`) → drop query-logs.test.ts from fixture-parity exclude (5 sibling PRs) |
