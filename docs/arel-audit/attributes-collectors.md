...
- TS: `Attribute extends NodeExpression` (~600 LOC). Each predicate method (eq/notEq/gt/gteq/lt/lteq/in/notIn/matches/doesNotMatch/matchesRegexp/doesNotMatchRegexp/between/notBetween/isDistinctFrom/isNotDistinctFrom/contains/overlaps/concat/`*Any`/`*All`/etc.) is implemented **directly on the class** rather than coming through the mixin. The Predications module is also imported.
...
- DRIFT: TS's Attribute extends Node (so it can act as a node in the AST); Rails' Attribute is a Struct that gets mixed into NodeExpression behavior. The TS approach is correct for the visitor dispatch but the LOC count is way above Rails because every mixin method has an explicit impl rather than a single `import { Predications }` mixin install.
- GAP: Confirm `lower()` and `typeCaster` getter present (Rails: `lower → relation.lower(self)`, `typeCaster → relation.typeForAttribute(name)`).
