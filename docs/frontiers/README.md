...
- **VirtualFS prepared statements:** `VirtualFS` builds SQL by interpolating escaped strings into queries. Migrate to prepared statements / bound parameters via the adapter for `path`, `content`, and `language` to eliminate injection risks and quoting bugs. The current single-quote escaping works but is brittle.
- **`isHunkApplied` for replace hunks:** When a replace hunk removes the anchor text, `isHunkApplied` can't detect it was applied (the anchor is gone). Needs a design decision: require anchors to survive replacement, or detect the inserted line sequence independently of the anchor.
